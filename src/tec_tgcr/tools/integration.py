...
class TECToolIntegration:
    ...
    def __init__(
        ...
        self.financial_monitor = AzureFinancialMonitor(
            subscription_id, self.data_path / "financial"
        )
    ...
    def daily_financial_check(self) -> Dict[str, Any]:
        ...
        try:
            ...
            result = {
                ...
                "anomalies": [
                    {"date": a.date, "cost": a.cost, "severity": a.severity}
                    for a in anomalies
                ],
                "actions_taken": [],
            }
            ...
            if critical_anomalies:
                logger.warning(
                    f"Found {len(critical_anomalies)} critical cost anomalies"
                )
                ...
                if evidence.total_unexpected_cost > 50.0:  # $50 threshold
                    ...
                    result["actions_taken"].append(
                        f"Submitted refund request: {request['ticket_id']}"
                    )

                    logger.info(
                        f"Auto-submitted refund request: {request['ticket_id']}"
                    )

            logger.info(
                f"Daily financial check completed: {len(anomalies)} anomalies found"
            )
            return result
    ...
    def process_new_evidence(self, case_id: Optional[str] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            result = {
                ...
                "critical_events": len(
                    [e for e in report.timeline if e.significance == "critical"]
                ),
    ...
    def get_evidence_status(self) -> Dict[str, Any]:
        ...
        try:
            ...
            audio_files = list(evidence_dir.glob("*.amr")) + list(
                evidence_dir.glob("*.mp3")
            )
...
def main():
    ...
    parser.add_argument(
        "--subscription-id", default="89d36e9a-a518-4151-95b3-087ec1b88ec5"
    )
    parser.add_argument(
        "--daily-check", action="store_true", help="Run daily financial check"
    )
    parser.add_argument(
        "--process-evidence", action="store_true", help="Process evidence files"
    )
    parser.add_argument(
        "--comprehensive", action="store_true", help="Run comprehensive check"
    )
    parser.add_argument(
        "--export-report", action="store_true", help="Export status report"
    )
    ...
    if args.daily_check:
        ...
        if result["financial_check"].get("anomalies_found", 0) > 0:
            print(
                f"Financial anomalies: {result['financial_check']['anomalies_found']}"
            )

        if result["evidence_processing"].get("events_processed", 0) > 0:
            print(
                f"Evidence events: {result['evidence_processing']['events_processed']}"
            )
